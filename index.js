...
// Store audio buffers in memory keyed by filename
...
fastify.all("/inbound", async (request, reply) => {
    ...
    try {
        ...
        // Generate unique filename and store the buffer
        const randomId = Math.random().toString(36).substring(2, 10);
        const filename = `audio-${randomId}.mp3`;
        audioStore.set(filename, buffer);
...
fastify.get("/audio/:filename", async (request, reply) => {
    ...
    const buffer = audioStore.get(filename);
